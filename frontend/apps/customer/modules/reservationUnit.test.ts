...
function constructPricing({
  ...
  return {
    id: "1",
    begins: formatApiDateUnsafe(begins ?? new Date()),
...
describe("getNextAvailableTime", () => {
  ...
  beforeEach(() => {
    ...
    reservableTimes.set(formatApiDateUnsafe(today), [
    ...
    reservableTimes.set(formatApiDateUnsafe(addDays(today, 1)), [
      {
  ...
  test("finds no available times if the duration is too long", () => {
    const today = new Date();
    const shortTimes = reservableTimes.get(formatApiDateUnsafe(today));
    if (!shortTimes) {
  ...
  test("Finds a date even if there are empty ranges before it", () => {
    ...
    reservableTimes.set(formatApiDateUnsafe(date), [
      {
  ...
  test("Finds a date after a requested date", () => {
    ...
    reservableTimes.set(formatApiDateUnsafe(date2), [
      {
