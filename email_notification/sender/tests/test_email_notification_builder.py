...
class EmailNotificationBuilderTestCase(TestCase):
    def setUp(self) -> None:
        ...
        self.reservation_unit = ReservationUnitFactory(
            name="Test reservation unit", unit=self.unit
        )
        self.reservation = ReservationFactory(
            name="Test reservation", reservation_unit=[self.reservation_unit]
        )
    ...
    def test_constructor_raises_error_on_invalid_html_content(self):
        html_file = SimpleUploadedFile(
            name="mock_file.html", content=b"HTML content FI {{invalid_tag}}"
        )
        template = EmailTemplateFactory(
            name="Test template",
    ...
    def test_get_content_with_html_file(self):
        html_file_fi = SimpleUploadedFile(
            name="mock_file_fi.html", content=b"HTML content FI"
        )
        html_file_en = SimpleUploadedFile(
            name="mock_file_en.html", content=b"HTML content EN"
        )
        html_file_sv = SimpleUploadedFile(
            name="mock_file_sv.html", content=b"HTML content SV"
        )
        ...
        builder_fi = ReservationEmailNotificationBuilder(
            self.reservation, template, "fi"
        )
        builder_en = ReservationEmailNotificationBuilder(
            self.reservation, template, "en"
        )
        builder_sv = ReservationEmailNotificationBuilder(
