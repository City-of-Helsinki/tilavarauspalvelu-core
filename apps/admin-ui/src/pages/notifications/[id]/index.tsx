...
const checkStartIsBeforeEnd = (
  ...
  const start = fromUIDateTime({ date: data.activeFrom, time: data.activeFromTime });
  const end = fromUIDateTime({ date: data.activeUntil, time: data.activeUntilTime });
...
const NotificationFormSchema = z
  ...
  .superRefine((x, ctx) => {
    if (!x.isDraft || x.activeFrom !== "" || x.activeFromTime !== "") {
      ...
      checkValidDate(fromUIDate({ date: x.activeFrom }), ctx, "activeFrom");
    }
  })
  ...
  .superRefine((x, ctx) => {
    if (!x.isDraft || x.activeUntil !== "" || x.activeUntilTime !== "") {
      ...
      if (!x.isDraft) {
        checkValidFutureDate(fromUIDate({ date: x.activeUntil }), ctx, "activeUntil");
      } else {
        checkValidDate(fromUIDate({ date: x.activeUntil }), ctx, "activeUntil");
      }
    }
  })
...
const NotificationForm = ({ notification }: { notification?: BannerNotificationPageQuery["bannerNotification"] }) => {
  ...
  const activeFrom = dateForInput({ date: notification?.activeFrom ?? today.toISOString() });
  const activeFromTime = notification?.activeFrom ? timeForInput({ time: notification?.activeFrom }) : "06:00";
  const activeUntil = notification?.activeUntil ? dateForInput({ date: notification?.activeUntil }) : "";
  const activeUntilTime = notification?.activeUntil ? timeForInput({ time: notification?.activeUntil }) : "23:59";
  ...
  const onSubmit = async (data: NotificationFormType) => {
    const end = fromUIDateTime({ date: data.activeUntil, time: data.activeUntilTime });
    const start =
      data.activeFrom !== "" ? fromUIDateTimeUnsafe({ date: data.activeFrom, time: data.activeFromTime }) : undefined;
