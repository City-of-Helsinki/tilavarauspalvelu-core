  ...
  const applicationSections = filterNonNullable(appEventsData?.applicationSections?.edges.map((e) => e?.node))
    .filter((section) => {
      const opts = section.reservationUnitOptions.filter((r) => {
        ...
        // FIXME this is awful
        const prefOrderFilter =
          queryVariables.preferredOrder == null
            ? []
            : Array.isArray(queryVariables.preferredOrder)
              ? (queryVariables.preferredOrder as Readonly<number[]>)
              : [queryVariables.preferredOrder];
        const preferredOrderFilter = prefOrderFilter.filter((x): x is number => x != null);
