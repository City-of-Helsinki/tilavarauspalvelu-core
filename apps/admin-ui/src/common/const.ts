...
export const API_BASE_URL = env.NEXT_PUBLIC_TILAVARAUS_API_URL != null
  ? `${env.NEXT_PUBLIC_TILAVARAUS_API_URL}`
  : '';

export const GRAPQL_API_URL = env.NEXT_PUBLIC_TILAVARAUS_API_URL != null
...
const AUTH_URL = env.NEXT_PUBLIC_TILAVARAUS_API_URL != null ? `${env.NEXT_PUBLIC_TILAVARAUS_API_URL}/helauth` : '/helauth';
const authUrl = AUTH_URL; // 'http://localhost:8000/helauth';

const getCleanPublicUrl = () => {
  const hasPublicUrl = publicUrl != null && publicUrl !== '/' && publicUrl !== '';
  const publicUrlNoSlash = publicUrl && hasPublicUrl ? publicUrl.replace(/\/$/, '') : '';
...
export const getSignOutUrl = (): string => {
  ...
  return `${authUrl}/logout?next=${callBackUrl}`;
};
