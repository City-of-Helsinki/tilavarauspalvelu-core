...
const COLS = [
  ...
  {
    headerTKey: "applicationSection:schedule.headings.time",
    isSortable: true,
    key: "allocated_time_of_week",
    transform: ({ time, link }: ApplicationScheduleView) => {
      if (link !== "") {
        return <TableLink href={link}>{time}</TableLink>;
      }
...
export function AllocatedSectionsTable({
  ...
  if (views.length === 0) {
    const name = t("applicationSection:allocated.emptyFilterPageName");
