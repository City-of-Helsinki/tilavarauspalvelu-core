...
import { type TimeStruct, toApiTime } from "../common/util";
...
describe("CreateStaffReservation schema", () => {
  ...
  /* TODO move this to separate check for the edges
  {
  ...
  */
  ...
  describe("15 min interval", () => {
    ...
    test.for(generateFuzzyIntegers(20, 0, 200, possibleMinutes))("%d is invalid for 15 min interval", (minutes) => {
      const tomorrow = addDays(new Date(), 1);
      const input = createInput({
        date: tomorrow,
  ...
  describe("half an hour interval", () => {
    ...
    test.for(generateFuzzyIntegers(20, 0, 200, possibleMinutes))("%d is invalid for 30 min interval", (minutes) => {
      const tomorrow = addDays(new Date(), 1);
      const input = createInput({
        date: tomorrow,
        start: { hours: 9, minutes },
