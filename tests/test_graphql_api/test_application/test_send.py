...
from tilavarauspalvelu.enums import ApplicantTypeChoice, OrganizationTypeChoice, Priority, Weekday
...
def test_send_application__community_applicant__org_type_improper(graphql):
    org = OrganisationFactory.create_for_community_applicant(organisation_type=OrganizationTypeChoice.COMPANY)
    ...
    assert response.has_errors is True
    assert response.field_error_messages() == [
        "Application organisation type is incorrect for the given applicant type.",
    ]
...
def test_send_application__association_applicant__improper_org_type(graphql):
    org = OrganisationFactory.create_for_association_applicant(organisation_type=OrganizationTypeChoice.COMPANY)
...
def test_send_application__company_applicant__improper_org_type(graphql):
    org = OrganisationFactory.create_for_company_applicant(
