...
from tests.test_graphql_api.test_reservation.helpers import ADJUST_STAFF_MUTATION, UPDATE_STAFF_MUTATION
...
def test_modify_staff_reservation__reservation_block_whole_day__ignore_given_buffers(graphql):
    ...
    reservation = ReservationFactory.create_for_reservation_unit(
        ...
        begin=datetime(2023, 1, 1, 12, tzinfo=DEFAULT_TIMEZONE),
        end=datetime(2023, 1, 1, 13, tzinfo=DEFAULT_TIMEZONE),
    ...
    response = graphql(UPDATE_STAFF_MUTATION, input_data=input_data)
...
@freezegun.freeze_time("2021-01-01")
def test_adjust_staff_reservation__reservation_block_whole_day(graphql):
    reservation_unit = ReservationUnitFactory.create(
        origin_hauki_resource=OriginHaukiResourceFactory(id=999),
        reservation_block_whole_day=True,
        spaces=[SpaceFactory.create()],
...
@freezegun.freeze_time("2021-01-01")
def test_adjust_staff_reservation__reservation_block_whole_day__ignore_given_buffers(graphql):
    reservation_unit = ReservationUnitFactory.create(
        origin_hauki_resource=OriginHaukiResourceFactory(id=999),
        reservation_block_whole_day=True,
        spaces=[SpaceFactory.create()],
