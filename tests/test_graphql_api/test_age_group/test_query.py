...
age_groups_query = partial(build_query, "ageGroups", connection=True, order_by="pk")
...
def test_age_group__filter__by_pk(graphql):
    # given:
    # - There are two age groups in the database
    ...
    # when:
    # - The user queries for age groups with a specific pk
    response = graphql(age_groups_query(pk=group_1.pk))

    # then:
    # - The response contains no errors
    # - The response contains only the selected age group
    assert response.has_errors is False, response
    assert len(response.edges) == 1, response
    assert response.node(0) == {"pk": group_1.pk}


def test_age_group__filter__by_pk__multiple(graphql):
    # given:
    # - There are two age groups in the database
    # - Regular user is using the system
    group_1 = AgeGroupFactory()
    ...
    # when:
    # - The user queries for age groups with specific pks
    response = graphql(age_groups_query(pk=[group_1.pk, group_2.pk]))

    # then:
    # - The response contains no errors
    # - The response contains both age groups
    assert response.has_errors is False, response
    assert len(response.edges) == 2, response
    assert response.node(0) == {"pk": group_1.pk}
