...
class BannerNotificationBuilder(ModelFactoryBuilder[BannerNotification]):
    ...
    def bold_messages(self) -> Self:
        message_fi = self.kwargs.get("message", self.kwargs.get("message_fi", self.factory.message()))
        message_en = self.kwargs.get("message_sv", self.factory.message_en())
        message_sv = self.kwargs.get("message_en", self.factory.message_sv())
