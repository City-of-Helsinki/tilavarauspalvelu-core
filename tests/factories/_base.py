...
class BaseFaker(Faker):
    ...
    def evaluate(self, instance: Resolver, step: BuildStep, extra: dict[str, Any]) -> Any:
        ...
        return self(unique=unique, **extra)

    def __call__(self, *, unique: bool = False, **kwargs: Any) -> Any:
        """
        Allows calling faker instances added to factories directly.
        ...
        ...         name = cls.name()  # Generates a random name here.
...
class ModelFactoryBuilder(Generic[TModel]):
    ...
    def build(self, **kwargs: Any) -> TModel:
        ...
        return self.factory.build(**self.kwargs)

    def create(self, **kwargs: Any) -> TModel:
        self.kwargs.update(kwargs)
        return self.factory.create(**self.kwargs)

    def build_batch(self, size: int, **kwargs: Any) -> list[TModel]:
        self.kwargs.update(kwargs)
        return self.factory.build_batch(size, **self.kwargs)

    def create_batch(self, size: int, **kwargs: Any) -> list[TModel]:
        self.kwargs.update(kwargs)
        return self.factory.create_batch(size, **self.kwargs)
