...
class ReservationWorkingMemoWriteTestCase(ReservationTestCaseBase):
    ...
    def test_working_memo_saves_when_admin(self):
        ...
        assert_that(confirm_data.get("workingMemo")).is_equal_to(input_data["workingMemo"])
        self.reservation.refresh_from_db()
        assert_that(self.reservation.working_memo).is_equal_to(input_data["workingMemo"])
    ...
    def test_working_memo_saves_with_general_comment_permission(self):
        ...
        comment_role_choice = GeneralRoleChoice.objects.create(code="can_comment_reservations")

        GeneralRolePermission.objects.create(role=comment_role_choice, permission="can_comment_reservations")
    ...
    def test_working_memo_saves_with_service_sector_comment_permission(self):
        ...
        comment_role_choice = ServiceSectorRoleChoice.objects.create(code="can_comment_reservations")

        ServiceSectorRolePermission.objects.create(role=comment_role_choice, permission="can_comment_reservations")
        ServiceSectorRole.objects.create(user=commenter, role=comment_role_choice, service_sector=self.service_sector)
    ...
    def test_working_memo_saves_with_unit_comment_permission(self):
        ...
        comment_role_choice = UnitRoleChoice.objects.create(code="can_comment_reservations")

        UnitRolePermission.objects.create(role=comment_role_choice, permission="can_comment_reservations")
    ...
    def test_working_memo_does_not_save_when_reserver_staff_user_and_not_own_reservation(
        ...
        response = self.query(self.get_update_memo_query(), input_data=self.get_valid_update_data())
        content = json.loads(response.content)
        assert_that(content.get("errors")).is_not_none()
        deny_data = content.get("data").get("updateReservationWorkingMemo")
        assert_that(deny_data).is_none()
