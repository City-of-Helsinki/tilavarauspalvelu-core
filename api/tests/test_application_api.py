...
def test_application_update_updating_and_adding_application_events(
    ...
    existing_event["application_event_schedules"] = [{"day": 3, "begin": "10:40", "end": "16:30"}]
    ...
    event = next(x for x in response.data.get("application_events") if x.get("id") == existing_event.get("id"))
    ...
    assert (
        next(x for x in response.data.get("application_events") if x.get("id") != existing_event.get("id")).get("name")
        == "New event name"
    )
...
def test_application_event_save(user_api_client, valid_application_event_data):
    ...
    response = user_api_client.post(reverse("application_event-list"), data=data, format="json")
...
def test_creating_weekly_amount_reduction_should_mark_declined(
    ...
    data = {"application_event_schedule_result_id": result_scheduled_for_monday.application_event_schedule.id}
...
def test_regular_user_cant_create_reductions(
    ...
    response = user_api_client.post(reverse("application_event_weekly_amount_reduction-list"), data, format="json")
...
def test_application_filter_by_units(event_reservation_unit, event_reservation_unit_too, user_api_client):
    ...
    response = user_api_client.get(f"{reverse('application-list')}?unit={unit_id_one}&unit={unit_id_two}")
    assert_that(response.status_code).is_equal_to(200)
    assert_that(response.data).is_length(2)
    assert_that(response.data[0]["id"] in [application_id_one, application_id_too]).is_true()
...
def test_application_filter_by_user(general_admin_api_client, application, user, user_2):
    ...
    response = general_admin_api_client.get(f"{reverse('application-list')}?user={user_2.id}", format="json")
