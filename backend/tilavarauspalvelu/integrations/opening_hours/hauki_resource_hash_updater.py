...
class HaukiResourceHashUpdater:
    ...
    def _process_single_hauki_resource(self, resource: HaukiAPIResource, *, force_refetch: bool = False) -> None:
        ...
        # Only delete existing ReservableTimeSpans if the hash has changed
        if origin_hauki_resource.opening_hours_hash != resource["date_periods_hash"]:
            self._discard_old_reservable_time_spans(origin_hauki_resource)

        self._update_resource_hash(origin_hauki_resource, resource)
        self._create_reservable_time_spans(origin_hauki_resource)

    def _update_resource_hash(self, origin_hauki_resource: OriginHaukiResource, resource: HaukiAPIResource) -> None:
        origin_hauki_resource.opening_hours_hash = resource["date_periods_hash"]
        origin_hauki_resource.latest_fetched_date = None  # Set to None, to clarify that the data is not up-to-date
        origin_hauki_resource.save()
        self.resources_updated.append(origin_hauki_resource)

    def _discard_old_reservable_time_spans(self, origin_hauki_resource: OriginHaukiResource) -> None:
        cutoff_date = local_date()  # Delete all new ReservableTimeSpans that start after this date.
        ...
        origin_hauki_resource.reservable_time_spans.filter(end_datetime__gte=cutoff_date).update(
            end_datetime=local_start_of_day()
        )
