...
class ReservationActions:
    ...
    def should_offer_invoicing(self) -> bool:
        ...
        if not accounting.actions.supports_invoicing():
            return False

        payment_types = set(reservation_unit.payment_types.values_list("code", flat=True))

        if not payment_types:
            return False

        return PaymentType.INVOICE in payment_types
