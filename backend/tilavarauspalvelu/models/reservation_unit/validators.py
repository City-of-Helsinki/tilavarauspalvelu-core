...
class ReservationUnitValidator:
    ...
    def validate_reservation_unit_is_published(self) -> None:
        if self.reservation_unit.publishing_state not in ReservationUnitPublishingState.states_that_are_visible:
            ...
            raise GraphQLValidationError(msg, code=error_codes.RESERVATION_UNIT_NOT_RESERVABLE)
    ...
    def validate_user_has_not_exceeded_max_reservations(self, user: User) -> None:
        ...
        if num_active_user_reservations >= self.reservation_unit.max_reservations_per_user:
            ...
            raise GraphQLValidationError(msg, code=error_codes.RESERVATION_UNIT_MAX_NUMBER_OF_RESERVATIONS_EXCEEDED)
    ...
    def validate_reservation_unit_is_direct_bookable(self) -> None:
        if self.reservation_unit.reservation_kind == ReservationKind.SEASON:
            ...
            raise GraphQLValidationError(msg, code=error_codes.RESERVATION_UNIT_NOT_DIRECT_BOOKABLE)
    ...
    def validate_begin_before_end(self, begin: datetime.datetime, end: datetime.datetime) -> None:
        if begin > end:
            ...
            raise GraphQLValidationError(msg, code=error_codes.RESERVATION_DURATION_INVALID)
    ...
    def validate_duration_is_allowed(self, duration: datetime.timedelta) -> None:
        ...
        if self.reservation_unit.max_reservation_duration and duration > self.reservation_unit.max_reservation_duration:
            ...
            raise GraphQLValidationError(msg, code=error_codes.RESERVATION_UNITS_MAX_DURATION_EXCEEDED)
        ...
        if self.reservation_unit.min_reservation_duration and duration < self.reservation_unit.min_reservation_duration:
            ...
            raise GraphQLValidationError(msg, code=error_codes.RESERVATION_UNIT_MIN_DURATION_NOT_EXCEEDED)
        ...
        if duration_minutes % interval_minutes > 0:
            ...
            raise GraphQLValidationError(msg, code=error_codes.RESERVATION_TIME_DOES_NOT_MATCH_ALLOWED_INTERVAL)
    ...
    def validate_reservation_begin_time(self, begin: datetime.datetime) -> None:
        if begin < local_datetime():
            ...
            raise GraphQLValidationError(msg, code=error_codes.RESERVATION_BEGIN_IN_PAST)
    ...
    def validate_reservation_days_before(self, begin: datetime.datetime) -> None:
        ...
        if max_days_before and max_allowed > datetime.timedelta(days=max_days_before):
            ...
            raise GraphQLValidationError(msg, code=error_codes.RESERVATION_NOT_WITHIN_ALLOWED_TIME_RANGE)
    ...
    def validate_not_in_open_application_round(self, begin: datetime.date, end: datetime.date) -> None:
        if self.reservation_unit.actions.is_in_open_application_round(begin, end):
            ...
            raise GraphQLValidationError(msg, code=error_codes.RESERVATION_UNIT_IN_OPEN_ROUND)
    ...
    def validate_has_payment_product(self) -> None:
        ...
        if not self.reservation_unit.payment_product:
            ...
            raise GraphQLValidationError(msg, code=error_codes.RESERVATION_UNIT_MISSING_PAYMENT_PRODUCT)
    ...
    def validate_has_payment_accounting(self) -> None:
        ...
        if not self.reservation_unit.actions.get_accounting():
            ...
            raise GraphQLValidationError(msg, code=error_codes.RESERVATION_UNIT_MISSING_PAYMENT_ACCOUNTING)
    ...
    def validate_cancellation_rule(self, begin: datetime.datetime) -> None:
        ...
        if cancel_rule is None:
            ...
            raise GraphQLValidationError(msg, code=error_codes.CANCELLATION_NOT_ALLOWED)
        ...
        if now > last_cancellable_moment:
            ...
            raise GraphQLValidationError(msg, code=error_codes.CANCELLATION_TIME_PAST)
    ...
    def validate_not_rescheduled_to_paid_date(self, begin: datetime.datetime) -> None:
        ...
        if pricing is None:
            ...
            raise GraphQLValidationError(msg, code=error_codes.RESERVATION_MODIFICATION_NOT_ALLOWED)
    ...
    def validate_can_create_reservation_type(self, reservation_type: ReservationTypeChoice) -> None:
        if reservation_type not in ReservationTypeChoice.types_that_staff_can_create:
            ...
            raise GraphQLValidationError(msg, code=error_codes.RESERVATION_TYPE_NOT_ALLOWED)
    ...
    def validate_has_access_type(self) -> None:
        if not self.reservation_unit.access_types.exists():
            ...
            raise GraphQLValidationError(msg, code=error_codes.RESERVATION_UNIT_HAS_NO_ACCESS_TYPE)
