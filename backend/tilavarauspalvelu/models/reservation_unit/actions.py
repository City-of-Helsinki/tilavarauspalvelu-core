...
from tilavarauspalvelu.models import OriginHaukiResource, PaymentProduct, Reservation
...
class ReservationUnitActions(ReservationUnitHaukiExporter):
    ...
    def refresh_reservation_unit_product_mapping(self) -> None:
        payment_merchant = self.reservation_unit.actions.get_merchant()

        if self.reservation_unit.actions.requires_product_mapping_update():
            ...
            self.reservation_unit.payment_product = payment_product
            self.reservation_unit.save(update_fields=["payment_product"])
    ...
    def refresh_reservation_unit_accounting(self) -> None:
        ...
        accounting = self.reservation_unit.actions.get_accounting()
        if not accounting:
            return
