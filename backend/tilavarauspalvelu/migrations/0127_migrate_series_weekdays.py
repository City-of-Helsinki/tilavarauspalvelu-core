...
def migrate_series_weekdays(apps, schema_editor):
    ...
    updated_series = list(ReservationSeries.objects.all())
    ...
    for series in updated_series:
        ...
        for weekday_num in series.weekdays:
            match weekday_num:
                ...
                case _:
                    msg = f"Invalid weekday for series {series.pk}: {weekday_num}"
                    raise ValueError(msg)

        series.new_weekdays = weekdays

    ReservationSeries.objects.bulk_update(updated_series, ["new_weekdays"])
