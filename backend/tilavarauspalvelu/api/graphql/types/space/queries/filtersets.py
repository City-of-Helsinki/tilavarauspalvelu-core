...
class SpaceFilterSet(FilterSet[Space]):
    pk = Filter(lookup="in")

    name_fi_exact = Filter("name_fi", lookup="iexact")
    name_sv_exact = Filter("name_fi", lookup="iexact")
    name_en_exact = Filter("name_fi", lookup="iexact")

    name_fi_contains = Filter("name_fi", lookup="icontains")
    name_sv_contains = Filter("name_fi", lookup="icontains")
    name_en_contains = Filter("name_fi", lookup="icontains")
    ...
    def only_with_permission(self, info: GQLInfo[User], *, value: bool) -> models.Q:
        ...
        return models.Q(unit__in=u_ids) | models.Q(unit__unit_groups__in=g_ids)
