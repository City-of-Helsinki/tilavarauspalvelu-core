...
class AllocatedTimeSlotCreateMutation(MutationType[AllocatedTimeSlot]):
    force = Input(input_only=True)
    ...
    def __validate__(cls, instance: AllocatedTimeSlot, info: GQLInfo, input_data: dict[str, Any]) -> None:
        force: bool = input_data.pop("force", False)

        option: int = input_data["reservation_unit_option"]
        section: ApplicationSection = ApplicationSection.objects.filter(reservation_unit_options=option).first()

        cls.validate_statuses(section=section)
        cls.validate_slots_per_week(section=section)
        cls.validate_reservation_unit_option_available(
