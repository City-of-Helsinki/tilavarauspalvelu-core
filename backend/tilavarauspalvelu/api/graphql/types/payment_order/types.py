...
class PaymentOrderNode(QueryType[PaymentOrder], auto=False, interfaces=[Node]):
    ...
    @Field
    def expires_in_minutes(root: PaymentOrder, info: GQLInfo) -> int | None:
    ...
    @Field
    def checkout_url(root: PaymentOrder, info: GQLInfo) -> str | None:
        if root.status not in OrderStatus.can_start_payment_statuses:
            return None
    ...
    @Field
    def receipt_url(root: PaymentOrder, info: GQLInfo) -> str | None:
        return root.receipt_url or None

    @expires_in_minutes.optimize
    def optimize_expires_in_minutes(self, info: GQLInfo, data: OptimizationData) -> None:
        data.only_fields |= {"status", "expires_at"}

    @checkout_url.optimize
    def optimize_checkout_url(self, info: GQLInfo, data: OptimizationData) -> None:
        data.only_fields |= {"checkout_url", "status", "expires_at"}

    @classmethod
    def __permissions__(cls, instance: PaymentOrder, info: GQLInfo) -> None:
