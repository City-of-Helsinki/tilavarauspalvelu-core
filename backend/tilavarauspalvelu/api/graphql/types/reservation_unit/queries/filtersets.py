...
from utils.db import NowTT, SubqueryArray, build_search
...
class AccessTypeFilterData(TypedDict):
    ...
    access_type_begin_date: NotRequired[datetime.date]
    access_type_end_date: NotRequired[datetime.date]
...
class ReservationUnitFilterSet(FilterSet[ReservationUnit]):
    ...
    def persons_allowed(self, info: GQLInfo[User], *, value: int) -> models.Q:
        less_than_min = models.Q(min_persons__lte=value) | models.Q(min_persons__isnull=True)
        more_than_max = models.Q(max_persons__gte=value) | models.Q(max_persons__isnull=True)
    ...
    equipments = Filter(many=True, match="any")

    @Filter
    def is_draft(self, info: GQLInfo[User], *, value: bool) -> models.Q:
        ...
        return is_published & is_visible if value else is_published & ~is_visible
    ...
    def is_visible(self, info: GQLInfo[User], *, value: bool) -> models.Q:
        ...
        return is_visible if value else ~is_visible
        ...
        return {
            "available_access_types": SubqueryArray(
                queryset=(
                    ...
                    .filter(
                        L(end_date__gt=value["access_type_begin_date"])
                        & models.Q(begin_date__lte=value["access_type_end_date"])
                    )
                    .values("access_type")
                ),
