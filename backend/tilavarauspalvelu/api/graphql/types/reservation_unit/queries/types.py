...
class ReservationUnitNode(
    ...
    @Field
    def payment_merchant(root: ReservationUnit, info: GQLInfo[User]) -> PaymentMerchant | None:
    ...
    def payment_merchant_optimizer(self, data: OptimizationData, info: GQLInfo[User]) -> None:
        payment_merchant_data = data.add_select_related("payment_merchant")
        payment_merchant_data.only_fields.add("id")

        unit_data = data.add_select_related("unit")
        unit_data.only_fields.add("id")

        unit_payment_merchant_data = unit_data.add_select_related("payment_merchant")
        unit_payment_merchant_data.only_fields.add("id")
    ...
    def hauki_url_permissions(root: ReservationUnit, info: GQLInfo[User]) -> None:
        user = info.context.user
        if not user.permissions.can_manage_unit(root.unit):
            msg = "No permission to access hauki url"
            raise GraphQLPermissionError(msg)
    ...
    def hauki_url_optimizer(self, data: OptimizationData, info: GQLInfo[User]) -> None:
        ...
        hauki_resource_data = data.add_select_related("origin_hauki_resource")
        hauki_resource_data.only_fields.add("id")
