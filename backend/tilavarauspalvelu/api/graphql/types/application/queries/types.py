...
class ApplicationNode(
    ...
    user = Field(ApplicantNode)

    @working_memo.permissions
    def working_memo_permissions(self, instance: Application, info: GQLInfo) -> None:
        user = info.context.user
        if not user.permissions.can_view_application(instance, reserver_needs_role=True):
            msg = "No permission to access working memo."
            raise GraphQLPermissionError(msg)

    @classmethod
    def __permissions__(cls, instance: Application, info: GQLInfo) -> None:
        user = info.context.user
    ...
    @classmethod
    def __filter_queryset__(cls, queryset: models.QuerySet[Application], info: GQLInfo) -> models.QuerySet[Application]:
        user = info.context.user
