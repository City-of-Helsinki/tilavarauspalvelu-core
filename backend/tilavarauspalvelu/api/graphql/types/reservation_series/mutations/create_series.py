...
class ReservationSeriesReservationCreateInput(MutationType[Reservation], kind="related"):
    ...
    # Time information
    buffer_time_before = Input(required=True, default_value=0)
    buffer_time_after = Input(required=True, default_value=0)
...
class ReservationSeriesCreateMutation(MutationType[ReservationSeries]):
    ...
    reservation_details = Input(
        ReservationSeriesReservationCreateInput,
        field_name="reservations",
        many=False,
        required=True,
    )
    check_opening_hours = Input(bool, default_value=False)
