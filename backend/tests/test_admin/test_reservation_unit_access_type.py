...
def test_reservation_unit_admin__access_types__set_to_past(api_client):
    ...
    assert errors == [
        {
            "code": "",
            "message": "Access type cannot be set before today.",
        },
    ]


def test_reservation_unit_admin__access_types__change_ongoing_begin_date(api_client):
    user = UserFactory.create_superuser()
    api_client.force_login(user)
...
def test_reservation_unit_admin__access_types__move_to_past(api_client):
    user = UserFactory.create_superuser()
    api_client.force_login(user)
...
def test_reservation_unit_admin__access_types__changing_to_access_type_checks_pindora(api_client):
    user = UserFactory.create_superuser()
    api_client.force_login(user)
...
@patch_method(PindoraClient.get_reservation_unit)
def test_reservation_unit_admin__access_types__changing_to_access_type_checks_pindora__not_if_already(api_client):
    user = UserFactory.create_superuser()
    api_client.force_login(user)
    ...
    data = {
        ...
        "access_types-0-id": access_type.id,
        "access_types-0-access_type": AccessType.ACCESS_CODE.value,
        "access_types-0-begin_date": (local_date() + datetime.timedelta(days=2)).isoformat(),
        #
        # Required fields
        **required_reservation_unit_form_data(reservation_unit),
        **management_form_data("access_types", total_forms=1, initial_forms=1),
...
def test_reservation_unit_admin__access_types__only_access_type_if_future_reservations(api_client):
    user = UserFactory.create_superuser()
    api_client.force_login(user)
    ...
    ReservationFactory.create(
        begin=combine(today + datetime.timedelta(days=2), local_time(12)),
        end=combine(today + datetime.timedelta(days=2), local_time(13)),
        access_type=AccessType.PHYSICAL_KEY,
        reservation_units=[reservation_unit],
    )
    ...
    assert errors == [
        {
            "code": "",
            "message": "Cannot delete last access type since there are already reservations after it becomes active.",
        },
    ]
