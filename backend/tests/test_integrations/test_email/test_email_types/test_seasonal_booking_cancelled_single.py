...
def test_seasonal_booking_cancelled_single__get_context(lang: Lang):
    ...
    with TranslationsFromPOFiles():
        context = get_context_for_seasonal_booking_cancelled_single(**get_mock_params(**params, language=lang))
...
@pytest.mark.parametrize("lang", ["en", "fi", "sv"])
@freeze_time("2024-01-01T12:00:00+02:00")
def test_seasonal_booking_cancelled_single__get_context__get_mock_data(lang: Lang):
    expected = LANGUAGE_CONTEXT[lang]
    ...
    with TranslationsFromPOFiles():
        context = get_mock_data(email_type=EmailType.SEASONAL_BOOKING_CANCELLED_SINGLE, **params, language=lang)
...
def test_seasonal_booking_cancelled_single__get_context__instance(email_reservation):
    ...
    with TranslationsFromPOFiles():
        context = get_context_for_seasonal_booking_cancelled_single(reservation=email_reservation, language="en")
...
@freeze_time("2024-01-01 12:00:00+02:00")
def test_seasonal_booking_cancelled_single__render__text():
    context = get_mock_data(email_type=EmailType.SEASONAL_BOOKING_CANCELLED_SINGLE, language="en")
    text_content = render_text(email_type=EmailType.SEASONAL_BOOKING_CANCELLED_SINGLE, context=context)
...
@freeze_time("2024-01-01 12:00:00+02:00")
def test_seasonal_booking_cancelled_single__render__html():
    context = get_mock_data(email_type=EmailType.SEASONAL_BOOKING_CANCELLED_SINGLE, language="en")
    html_content = render_html(email_type=EmailType.SEASONAL_BOOKING_CANCELLED_SINGLE, context=context)
...
@freeze_time("2024-01-01 12:00:00+02:00")
def test_seasonal_booking_cancelled_single__send_email(outbox):
